...
from data import AnnotationTransform, AnimeDetection, detection_collate, preproc, cfg
...
def train():
    ...
    dataset = AnimeDetection(filelist, preproc(img_dim, rgb_mean), AnnotationTransform())

    epoch_size = math.ceil(len(dataset) / batch_size)
    max_iter = max_epoch * epoch_size

    #stepvalues = (200 * epoch_size, 250 * epoch_size)
    stepvalues = (50 * epoch_size, 100 * epoch_size)
    step_index = 0
    ...
    for iteration in range(start_iter, max_iter):
        ...
        print('{}/{}|{}/{}|{}/{}|L:{:.4f} C:{:.4f}|LR:{:.8f} |Batchtime:{:.4f}s|ETA:{}'.format(epoch, max_epoch, (iteration % epoch_size) + 1, epoch_size, iteration + 1, max_iter, loss_l.item(), loss_c.item(), lr, batch_time, str(datetime.timedelta(seconds=eta))))
